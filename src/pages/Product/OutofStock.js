...
const OutofStock = () => {
  ...
  useEffect(() => {
    const apiUrl = `http://127.0.0.1:8000/api/product/outOfStock?page=${currentPage}&per_page=${itemsPerPage}&search=${search}`;
    const config = {
      headers: {
  ...
  const handleSwitchToggle = async(id) => {
    ...
    try {
     ...
     // console.log(response)
      showSuccessAlert('Product '+response.data.data.prod_name+' status updated successfully');
      setrecallApi((prev) => !prev);
    } catch (error) {
    ...
    <div>
      {
        ...
        /></div> : (   <div>
          <Banner title={"View Out of Stock Products"}/>
          <div className='home'>
